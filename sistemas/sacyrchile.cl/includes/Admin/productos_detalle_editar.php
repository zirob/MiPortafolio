...
if(isset($_GET['action'])){    
    if($_GET['action']==2 && isset($_GET['new']) && $_GET['new']==2){
        ...
        $sql_even = "INSERT INTO eventos (usuario, rut_empresa, tabla_evento, id_registro_tabla_evento, tipo_evento ";
            ...
            $sql_even.= ", 'UPDATE:codigo_interno=".$cod_interno.",especifico=".$especifico.",
            patente=".$patente.",agno=".$anio.",color=".$color.",marca=".$marca.",
            modelo=".$modelo.",potencia_KVA=".$potencia.",horas_mensuales=".$horas_mensuales.",
            consumo_nominal=".$consumo_nominal.",consumo_mensual=".$consumo_mensual.",
            peso_bruto=".$peso_bruto.",referencia_sacyr=".$referencia.",vin_chasis=".$vin.",
            ...
            observaciones=".$observaciones.",
            estado_producto=".$estado.",producto_arrendado=".$arrendado.",empresa_arriendo=".$emp_arriendo.",id_oc=".$id_oc."', ";
        $sql_even.= "'".$_SERVER['REMOTE_ADDR']."', 'Update en preinventario', '1', '".date('Y-m-d H:i:s')."') ";
        mysql_query($sql_even, $con); 
       
        if(mysql_query($sql_update,$con)){
            $msg ="Se han actualizado correctamente los datos";
        }else{
            $msg="Ha ocurrido un error al actualizar intente mas tarde..".$sql_update;
        }
    }
}
                ...
                <tr>
                    ...
                    <td><label>Estado:</label>
                        <select name="estado">
                            ...
                            <option value="2" <?if($estado==2){ echo "SELECTED";}?>>Asignado</option>
                            <option value="3" <?if($estado==3){ echo "SELECTED";}?>>Dado de Baja</option>
                            <option value="4" <?if($estado==4){ echo "SELECTED";}?>>En Mantenci&oacute;n</option>
                            <option value="5" <?if($estado==5){ echo "SELECTED";}?>>En Reparaci&oacute;n</option>
                        </select>
                    </td>
                </tr>    
                <tr>
