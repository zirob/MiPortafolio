...
if(isset($_GET['action']) && $_GET['action']==2){
    ...
    if(($_POST['dia_entrega']<=31 && $_POST['dia_entrega']>0) && ($_POST['mes_entrega']<=12 && $_POST['mes_entrega']>0) && (date('Y',  strtotime($_POST['anio_entrega']))>=date('Y'))){   
            $fecha_entrega = $_POST['anio_entrega']."-".$_POST['mes_entrega']."-".$_POST['dia_entrega'];
             ...
             $sql = "SELECT * FROM cabecera_oc WHERE id_oc=".$id;
                    ...
                    $vb_j_pm = $row['vb_jefe_parque_maquinaria'];
                      ...
                      $plan_calidad=$row['sometido_plan_calidad'];
                       $esp_tecnica =$row['adj_esp_tecnica'];
                        $prop_econ=$row['adj_propuesta_economica'];
                    
                    $action="2&id_oc=".$_GET['id_oc'];
         }
    }else{
        $error="La Fecha de Entrega Ingresada es Invalida";
    }     
...
if(isset($_GET['action'])  && $_GET['action']==1  && $_POST['solicitud_oc']!=""){
    ...
    if(isset($_POST['vb_depto_compras']) && $_POST['vb_depto_compras']!="" && isset($_POST['vb_jefe_compras']) && $_POST['vb_jefe_compras']!="" && isset($_POST['vb_jefe_adm']) && $_POST['vb_jefe_adm']!="" && isset($_POST['vb_jefe_pm']) && $_POST['vb_jefe_pm']!=""){
        ...
        if(($_POST['dia_entrega']<=31 && $_POST['dia_entrega']>0) && ($_POST['mes_entrega']<=12 && $_POST['mes_entrega']>0) && (date('Y',  strtotime($_POST['anio_entrega']))>=date('Y'))){   
            ...
            $sql_ins="INSERT INTO cabecera_oc (id_solicitud_compra,rut_empresa,fecha_oc,solicitante,concepto,centro_costos,moneda,
                ...
                VALUES ('".$_POST['solicitud_oc']."','".$_SESSION['empresa']."','".date('Y-m-d')."','".$_POST['solicitante']."','".$_POST['concepto']."','".$_POST['centro_costo']."','".$_POST['moneda']."'
                    ,'".$_POST['subtotal']."','".$_POST['descuento']."','".$_POST['valor_neto']."','".$_POST['iva']."','".$_POST['total_pago']."','".$_POST['forma_de_pago']."','".$fecha_entrega."','...'
        ...
        $error="Ha ocurrido un error al grabar datos intente mas tarde";
            ...
            $error="La Fecha de Entrega es Invalida ";
        }
}
...
<table style="width:1000px;" cellpadding="3" cellspacing="4">
            ...
            <tr>
                ...
                <td colspan="2" style="text-align: left;">
                    <table>
                        <tr><td><input class="fu" type="text" size="50" name="solicitante" value="<?=$solicitante;?>"></td></tr>
                        <tr><td><select name="concepto" class="fu" style="width:330px;" >
                        <option value=""> --- </option>
                        ...
                        <tr>
                            <td>
                               <select name="centro_costo"  class="fu" style="width:330px;">
                                <option value=""> --- </option>
                                <?
                        ...
                        <tr>
                            <td >
                                <select name="solicitud_oc" onchange="solic_oc_select(this.value)"  class="fu" style="width:330px;">
                               ...
                               $r = mysql_query($s,$con);
                                if(mysql_num_rows($r)!= NULL){
                                    ...
                                    <option value="<?=$rw['id_solicitud_compra'];?>" <?if($id_solic_oc==$rw['id_solicitud_compra']){ echo "SELECTED";}?>><?=$rw['descripcion_solicitud'];?></option>
                                <?    }
                                }   
                                ?>
                                </select>
                    ...
                    <table width="100%"  style="width:100%; border:1px solid;" cellspacing="3" cellpadding="3" >
                        <tr>
                            <td height="100%;" id="proveedor_selec"><b>Proveedor:</b>
                               <select name="proveedor" style="width:500px;" class="fu" onchange="selecciona_proveedor(this.value)">
                                    <option value=""> --- </option>
            ...
            <table style="width:100%; border:1px solid;" id="detalle-prov" class="detalle-oc"  cellpadding="3" cellspacing="4">
                ...
                <? if($can>0){ 
                    ...
                    <td><input class="fu nume" type="text" name="cant[]" id="cant_<?=$i;?>" size="4"  value="<?=number_format($rew['cantidad'],2,',','.');?>"></td>
                    ...
                    <td><input class="fu nume" type="text" name="precio[]"  id="precio_<?=$i;?>" size="9"  value="<?=number_format($rew['precio'],2,',','.');?>"></td>
                    <td style="background-color: #fff;"><input class="fu nume" type="text" id="total_<?=$i;?>" name="total[]"  value="<?=number_format($rew['total'],2,',','.');?>" readonly="readonly"  size="10"></td>
                    <td><td style="text-align:center;"><? if($i!=1){?><a href="javascript:eliminar_detalle('<?=$i;?>');"><img src="img/delete2.png" width="16px" height="16px"></a><? } ?></td>'</td>
                </tr>
                <?
                    $i++;
                    }
                ...
                <tr><td colspan="7"><a href="javascript:agregar_detalle_oc(1);" id="agregar"><img src="img/add1.png" width="20px" height="20px"></a>
                ...
                <tr>
                 ...
                 <tr id="det_1" class="det" >
                    ...
                    <td><input class="fu nume" type="text" name="cant[]" id="cant_1" size="4"></td>
                    ...
                    <td><input class="fu nume" type="text" name="precio[]"  id="precio_1" size="9" ></td>
             ...
             <tr>
                ...
                <td colspan="2" rowspan="3">
                    <table>
                        <tr>
                            <td><b>Subtotal</b></td>
                            <td><input class="fo" type="text" id="subtotal" name="subtotal" value="<?=number_format($subtotal,2,',','.');?>" readonly="readonly"  style="background-color: #fff;color:#000;"></td>
                        </tr>
                        <tr>
                            <td><b>Descuento</b></td>
                            <td><input class="fo nume" type="text" name="descuento" value="<?=number_format($descuento,2,',','.');?>" id="descuento_oc" ></td>
                        </tr>
                        <tr>
                            <td><b>Valor Neto</b></td>
                            <td><input class="fo nume" type="text" id="valor_neto_oc" name="valor_neto" readonly="readonly" value="<?=number_format($valor_neto,2,',','.');?>"  style="background-color: #fff;color:#000;"></td>
                        </tr>
                        <tr>
                            <td><b>I.V.A %</b></td>
                            <td><input class="fo nume" type="text" name="iva" id="iva" onchange="calculo_total();" value="<?=number_format($iva,0,'','.');?>"></td>
                        </tr>
                        <tr>
                            <td><b>Total</b></td>
                            <td><input class="fo" type="text" id="total_pago" name="total_pago" readonly="readonly" value="<?=number_format($total,2,',','.');?>"  style="background-color: #fff;color:#000;"></td>
                        </tr>
                        <tr>
                            <td colspan="2"><input type="button" value="Calcular Total OC" style="float:right;" onClick="javascript:calcular_cambios_oc(<?=$can;?>);"></td>
                        </tr>
                    </table>
                </td>
            </tr>
                    ...
                    <table  width="100%">
                        ...
                        <tr>
                            <td style="height: 100px; width: 200px;border: 1px solid;">
                                <b>VB Depto Compras</b><br/><input type="checkbox" name="vb_depto_compras" <?if((int)$vb_dc==1){ echo "checked";}?>>
                            </td>
                            <td style="height: 100px; width: 200px;border: 1px solid;">
                                <b>VB Jefe Compras</b><br/><input type="checkbox" name="vb_jefe_compras" <?if((int)$vb_jc==1){ echo "checked";}?>>
                            </td>
                            <td style="height: 100px; width: 200px;border: 1px solid;">
                                <b>VB Jefe ADM</b><br/><input type="checkbox" name="vb_jefe_adm" <?if((int)$vb_j_adm==1){ echo "checked";}?>>
                            </td>
                            <td style="height: 100px; width: 200px;border: 1px solid;">
                                <b>VB Jefe PM</b><br/><input type="checkbox" name="vb_jefe_pm" <?if((int)$vb_j_pm==1){ echo "checked";}?>>
                            </td>
                            ...
                            <td style="height: 100px; width: 100px"><b>Estado Orden de Compra</b><br />
                                <select name="estado" <?if($estado==4 || !isset($_GET['id_oc'])){ echo "DISABLED";}?>>
                                    ...
                                    <option value="2" <?if($estado==2){ echo "SELECTED";}?>>Pendiente</option>
                                    <option value="3" <?if($estado==3){ echo "SELECTED";}?>>Cerrada</option>
                                    <option value="4" <?if($estado==4){ echo "SELECTED";}?>>Aprobada</option>
                                    <option value="5" <?if($estado==5){ echo "SELECTED";}?>>Rechazada</option>
                                </select>
